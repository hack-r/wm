...
train$FinelineNumber[is.na(train$FinelineNumber)] <- Mode(train$FinelineNumber)
...
#saveRDS(test, "test_xmas.RDS")
#write.csv(test, "test_xmas.csv", row.names = F)
...
# Model -------------------------------------------------------------------
bst          <- xgboost(data = sparse_matrix,label = Y3, max.depth = 6, eta = .01, nround = 3000, #ntree = 100, #,
...
write.csv(sub, 'xgboost_pred_3k.csv', quote = F, row.names = F)
zip("xgboost_pred.zip", "xgboost_pred_3k.csv")
